...
namespace Parser {
  ...
  {
  protected:
    virtual Transition input_state_rule( unichar_t ch ) const = 0;
    StateFamily *family;

  private:
    Transition anywhere_rule( unichar_t ch ) const;

  public:
    void setfamily( StateFamily *s_family ) { family = s_family; }
    Transition input( unichar_t ch ) const;
  ...
  class Ground : public State {
    Transition input_state_rule( unichar_t ch ) const;
  };
